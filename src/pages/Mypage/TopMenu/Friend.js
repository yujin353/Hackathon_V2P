...
const Friend = () => {
    ...
    useEffect(()=>{
        $.ajax({
            ...
            success: (response) => setFollowerList(response.results), //mem = response.results[0].follower_member['member_id']
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    for (let i = 0; i < userlist.length; i++) {
                        if (userlist[i].username == textInput) {
                            setUserId(userlist[i].member_id);
